...
| A manifest-completeness gate walked shipped files with `find -type f`, which never matches a symlink — a file shipped as a symlink was invisible to the check meant to catch it (#938). Three test suites carried 21 sites where an early-exiting consumer under `set -o pipefail` could SIGPIPE-kill a still-writing producer; one was a bare assignment that aborted the whole run partway through, so a truncated run still read green (#945). Bash's builtin `echo` flag-parses an argument of exactly `-n`/`-e` into empty output — what #946 filed as 7 sites became 13 once the sweep widened past piped `tr` calls. | `find -L` to follow symlinks; herestrings in place of piped consumers under `pipefail`; `printf` in place of `echo` for flag-shaped arguments |
