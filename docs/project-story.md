...
### September 2026 (through September 9) — Cloudflare reversal, the advisor-guard's shell-parsing model, and the ask-context gate's blind spot (15 commits)
...
**The September rewrite (2026-09-10).** Anthropic's post on reducing cost with the Claude Platform put numbers on what the discipline wave had been costing: every rule loads into every session and every subagent, frontier models follow emphasis and rituals literally, and contradictory instructions are followed arbitrarily. Measured on one machine, the rules were 86.5k of a 120k-token startup prefix and a delegating session spawned a median of ten subagents. Five PRs (#1064 through #1068) turned attribution off in settings, deduplicated hook registrations, set one-hour cache TTLs, put `effort:` on every agent, resolved six contradictions by deleting the losing side, gave twelve stack rules `paths:` frontmatter, moved seven reference rules to skills, and rewrote the 22 always-loaded rules without the Iron Law header, rationalization table, red flags, or announce line. `rule-authoring.md` now asks for one plain-sentence rule, the mechanism, and the facts. The startup prefix fell from 120k to 55k tokens.
...
| The ask-context gate blocked a question twice after a full context brief: on Claude Fable 5.1, text written between tool calls is a summarized progress-update `thinking` block, never a `text` block, so the transcript count read 86 characters against a ~1,100-character brief the user saw as one line | G3 measures the AskUserQuestion payload (question text, option descriptions, previews) as the primary surface, with the transcript count as a second way to pass; the rule now tells agents mid-turn text is not a verbatim surface |
