...
**What it is.** A per-session posture for expensive orchestrator models (Fable/Opus): `/advisor on` turns the main agent into a pure delegator — it writes specs, dispatches cheaper implementer agents, reviews their diffs through the standard two-stage separate-agent review, triages findings, delegates fixes, and merges, but never edits source itself. `/etp` already runs this loop for plan- and issue-shaped work; advisor mode routes ad-hoc requests through the same discipline instead of letting the orchestrator implement inline. Pilot X-Plan/adrev/ETP workflows use a three-round artifact-fix checkpoint with evidence-backed extensions under unchanged total budgets. Other callers retain the hard three-round limit.
