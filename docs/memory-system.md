...
Both calls send `output_config.format` with a JSON schema, so the response shape is enforced by the API rather than requested in prose. `max_tokens` is a backstop at 16000, not a tuning knob: a response that stops at the cap is reported as a failed extraction on stderr and counted in the run summary the digest renders, never accepted as a short answer. The preflight prices a call at a separate planning figure, so raising the backstop does not shrink the plan.
