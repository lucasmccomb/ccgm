...
**What it does**: At question time the only surface guaranteed to reach the user verbatim is the question payload itself — question text, option descriptions, previews. Plain assistant text counts only where it opens or ends the turn: on Claude Fable 5.1, text written between tool calls comes back as a summarized progress update and is stored as `thinking`, and the agent's thinking and collapsed tool output are invisible. This hook hard-blocks (exit 2) AskUserQuestion calls that violate that model, with three gates: **deictic references** in the payload to context the user cannot see ("with that context", "as described above", "see above"); **identical re-asks** of a question the user answered in free text (Other), dismissed, or never answered — the "same question again" loop — while re-asks after a picked option stay allowed for approval loops; and **invisible context**, asking mid-workstream (≥1 tool call since the user's last message) when the payload carries under 200 characters of context and under 200 characters of assistant text reached the transcript this turn (`ASK_CONTEXT_MIN_CHARS` overrides both). Harness-injected user entries (skill expansions, system-reminder wrappers, local-command output) do not start a turn. Each denial message contains the recovery recipe: put the context in the payload, then re-call. Transcript gates fail open on unreadable transcripts; `CCGM_ASK_CONTEXT_OFF=1` escape hatch.
...
Codebase system map: `/orrery` deep-dives a repo with parallel read-only scout agents and renders an interactive, zoomable, embeddable system-design map as one self-contained HTML file.

**Installs**: `skills/orrery/SKILL.md` (`/orrery`), `agents/orrery-scout.md` (the restricted Read/Grep/Glob-only scout), fragment/model JSON schemas and pack briefs under `skills/orrery/references/`, and eight deterministic scripts under `skills/orrery/scripts/` (anchor, census, diff, merge, emit, validate, render, and the lockfile-pinned LikeC4 toolchain entry point)
...
- **Phase 4** - Personal lead review of security, architecture, and business logic (review stage 1 of 2)
- **Phase 5 (+5.6)** - Write plan.md, then a self-review loop for placeholders, identifier drift, and autonomous-execution readiness
- **Phase 5.7** - The lead performs N selected adversarial passes; every pass covers the entire plan and its four execution tenets, and pass N is final. Explicit `--cross-provider` or natural-language opt-in enables opposite/origin/opposite native review after an early local binary/login preflight.
...
**What it does**: `/adrev` resolves a plan, doc, PR, issue, code directory or concept, then the lead personally reviews it by default. Explicit `--cross-provider` or natural-language opt-in routes a fresh restricted reviewer opposite the actual producer. It attacks premises, failure modes, strongest opposing arguments, falsifiability and reversal costs, plus the four plan-execution tenets where relevant. Supported plan changes go to one designated writer after evidence-backed lead triage; an active optional run uses its critic/disposition contract. `--no-apply` and report-only preserve the target; headless applies only with explicit `--apply`. Report completion with open findings does not imply consensus. In optional native mode, unknown/mixed authorship requires both perspectives.
...
Opt-in local Claude Code/Codex review with explicit producer routing, frozen hashes, validated findings, bounded calls and a self-contained Codex skill. X-Plan still chooses 1–3 reviews upfront (default one); the lead performs selected plan passes and ETP spec/quality review personally by default. Explicit native runs add ordered stages and acknowledgment gates after local binary/login preflight. Provider errors stop only that optional run, preserving evidence; a separate lead release decision does not label it approved. Private runs use `~/.claude/cross-agent-review/<run-id>/`; advisor mode permits only the installed policy shim, unavailable in marketplace-only Claude installations. Install with `bash start.sh --add cross-agent-review`; optional Codex setup uses `python3 ~/.claude/bin/cross-agent-review-setup.py install`. See [module documentation](../modules/cross-agent-review/README.md).
