...
**Installs**: `skills/orrery/SKILL.md` (`/orrery`), `agents/orrery-scout.md` (the restricted Read/Grep/Glob-only scout), fragment/model JSON schemas and pack briefs under `skills/orrery/references/`, and seven deterministic scripts under `skills/orrery/scripts/` including the lockfile-pinned LikeC4 toolchain
