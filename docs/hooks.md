...
Hard gate ensuring every AskUserQuestion carries visible decision context. The one surface the user always sees verbatim is the question payload; plain assistant text reaches them only where it opens or ends the turn (on Claude Fable 5.1, text between tool calls arrives as a summarized progress update stored as `thinking`), and thinking and collapsed tool output are invisible — so an agent that "analyzed it", or even wrote a brief mid-turn, has not necessarily shown it. Three gates: **deictic references** in the question text or option descriptions to context outside the payload ("with that context", "as described above", "see above"); **identical re-asks** of a question set the user answered in free text (Other), dismissed, or never answered — re-asks after a picked option label stay allowed for recurring approval loops; and **invisible context** — asking mid-workstream (≥1 tool call since the user's last real message; harness-injected user entries do not count as one) when the payload (question text + option descriptions + previews) carries fewer than 200 characters of context and fewer than 200 characters of assistant text reached the transcript this turn (`ASK_CONTEXT_MIN_CHARS` overrides both). Every denial message contains the recovery recipe: put the context in the payload, then re-call. Transcript-based gates fail open when the transcript is unreadable; the payload gate always runs.
