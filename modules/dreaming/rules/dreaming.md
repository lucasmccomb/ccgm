...
An arm run that failed to execute is never sent to the judge: there is nothing to grade, the untouched fixture would score whatever it already scored (10.0 on a canary task, for a run that did no work), and on a broken harness that is a whole task's judge calls spent before the abort fires. The run carries a 0.0 instead, and `format_error_rate` carries the reason.
