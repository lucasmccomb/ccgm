...
- **A whole-run format-error rate of 1.0 aborts.** One failed arm run stays non-fatal (it is recorded and the eval carries on); a run where *every* arm failed is not a measurement, so the harness prints the first failure's raw output to stderr, exits non-zero, and writes no results file. `--gate` then reports `no results` — "the harness never ran" — instead of a regression nobody measured.
