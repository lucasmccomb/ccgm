...
| R9 | **Redacted-long-secret window under-count** — a legitimate excerpt whose redaction placeholder stands in for a raw secret *longer* than the placeholder literal could fail to corroborate | The comparison window was sized to the placeholder-kept excerpt length, shorter than the raw span whenever the secret exceeded its ~14-25-char placeholder | **Fixed with bounded slack (#846).** Each `[REDACTED:kind]` placeholder now extends the window by `_MAX_REDACTED_SECRET_LEN` (200) chars — bounded + per-placeholder, never transcript-proportional, applied ONLY for redacted excerpts, with guard (ii)'s token floor and the ratio/jaccard thresholds unchanged. **Remaining residual (honest):** a secret *longer than 200 chars* (authorization_bearer JWTs, long `env_var_kv` values — the unbounded-upper patterns) still under-counts. That case **fails closed** (session unverified → row stays `pending` for `/dream-apply`); a safety under-admission, never an over-admission. The 200-char cap is a plausible-length allowance, not a guarantee | Issue **#846** (FIXED, bounded-slack); `test_eligibility_gate.py::RedactionWindowSlackTests::{test_long_redacted_secret_now_corroborates, test_long_redacted_secret_rejected_without_slack, test_placeholder_free_window_unaffected_by_slack, test_slack_cannot_clear_coincidence_guard_ii_still_binds, test_slack_cap_pinned}` |
...
| Excerpt anti-coincidence guards (size-independence) | `test_eligibility_gate.py::GuardIITwoSidedTests::{test_short_common_rejected_and_redacted_legit_accepted, test_proportional_arm_binds_beyond_absolute_floor}` |
