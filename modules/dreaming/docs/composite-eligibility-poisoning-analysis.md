...
- **Circuit breaker** — unchanged: `test_optimistic_engine.py::CircuitBreakerTests::test_breaker_trips_when_two_anomalies_fall_within_window`, `test_optimistic_engine.py::AutoResumeCleanSlateTests::test_breaker_auto_resumes_after_quiet_period`.
...
| R9 | **Redacted-long-secret window under-count** — a legitimate excerpt whose redaction placeholder is *shorter* than the raw secret it replaces (a real `ghp_…` token of 34-40 chars vs a ~21-char `[REDACTED:github_token]` placeholder) can fail to corroborate | The comparison window is sized to the placeholder-kept excerpt length, which is shorter than the raw span when the secret exceeds its placeholder | **Fails closed** (session not verified → row stays `pending`); a safety residual, not a security defect — it silently *under*-admits security-relevant corrections. Tracked, validated in H1's real-data replay | Issue **#846** (OPEN); short-secret cases pass today: `test_eligibility_gate.py::SessionVerificationMatrixTests::test_redaction_placeholder_excerpt_still_counts` |
