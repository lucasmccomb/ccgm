...
| Triage reviewer findings; coordinate evidence-based resolution | Commit, push, stash, or apply patches |
...
4. **Review** through the standard two-stage separate-agent review (`spec-compliance-reviewer`, then `code-quality-reviewer`). When executing the cross-agent pilot, use its actual-producer routing and frozen-evidence policy instead; the restricted reviewer requests independently executed checks rather than running tools itself. The reviewer gets the spec and the diff — never the implementer's rationale or self-report as grounding. Give reviewers explicit success criteria and require cited evidence (commands run, outputs observed): a reviewer without criteria rubber-stamps.
5. **Triage** findings and dispatch supported fixes. For the cross-agent pilot callers (X-Plan, adrev and ETP), use their policy: opposite-provider evidence and acknowledgments close substantive findings; orchestrator authority cannot waive an objection. Three fix rounds are the checkpoint; a further bounded batch requires recorded new evidence and a viable next check without resetting overall limits. Outside those callers, retain the existing max-three-fix-round freeze/record/escalate rule.
...
- A fourth fix round on the same unit without the pilot policy's explicit evidence-backed extension (non-pilot callers retain the hard three-round bound)
