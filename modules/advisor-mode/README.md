...
Main-agent execution requires the installed shim and its local imports to resolve outside advisor write roots, typically through canonical repository symlinks; copied installations require delegation. A marketplace-only Claude install lacks the trusted shim. Use canonical CCGM installation in symlink mode or delegate setup/execution under normal permissions; do not broaden the guard. Optional provider errors stop that run and preserve reports/findings. The lead may separately judge delivery using evidence and normal checks, without calling the stopped run approved. Coordinator repairs require no recursive provider consensus. The optional native run has a hard two-correction-cycle cap, three exchanges per stage and no extensions. Ordinary lead review retains its separate three-round checkpoint; further lead work needs new evidence and a viable next check. Check records and host receipts are attestations, not independently proven execution/reception.
