...
- **Phase 5.7** - Adversarial review loop (stage 2 of 2): ≥3 hostile `adrev-reviewer` agents on Opus 4.8 (max effort) attack the finished plan, findings are incorporated, then it re-attacks until the plan survives a clean round (cap 3 rounds). A finished plan has had **≥6 independent reviews**.
...
|------|-----------|----------|------------|-------|---------|-------------|
| Default (interactive) | Full Q&A | Full | Approved by user | Approved by user | Standard + **adversarial loop** | Skipped (approved inline) |
| `--light` | Skipped | Reduced (inferred) | Internal default | Internal | Optional (no adversarial loop) | Full section-by-section at end |
| `--autonomous` (or `/xplana`) | Skipped | **Full** | Internal (best-fit) | Internal (best-fit) | **Full standard + adversarial loop (always)** | Plan-as-artifact presentation at end |

Reviews run in two stages: Phase 4 standard peer review against the draft, then the Phase 5.7 adversarial loop against the finished plan. A completed plan has survived **≥6 independent reviews** (3 standard + ≥3 adversarial).
...
- **adversarial-review**: Provides the `adrev-reviewer` agent that Phase 5.7's adversarial review loop dispatches (≥3 hostile reviewers on Opus 4.8). Installed automatically as a module dependency.
