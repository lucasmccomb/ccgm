...
3. **Daily analyzer** (Epic 6). A launchd job runs `bin/autoheal-daily.sh` at 08:00 local (UTC-keyed; see #525). The analyzer reads unanalyzed events, clusters routine successes by `(tool, command-prefix)` so heavy days fit under the input cap, keeps friction records (deny/ask permission_decisions, non-zero exits, corrections) as full records, and adapts the transcript excerpt window (3 → 1 → 0 turns) before rejecting on size. It calls the Anthropic API directly (no agent runtime — eliminates exec-escape attack surface) and writes proposals to `~/.claude/autoheal/proposals/{YYYY-MM-DD}.jsonl`. The call runs on the configured model (`default_model` in config.json, else `claude-sonnet-5`) with thinking disabled at `effort: low` and an `output_config.format` JSON schema built from `lib/proposal-schema.json`, all set explicitly in the request body — the schema makes the response shape the API's job, and the explicit thinking setting is what keeps a model that thinks by default from spending the output budget before the JSON. A configured model that does not support structured outputs stops the run before any call, naming the model and listing the ones it can use. That list and the `cost_pricing` tables are one contract: a model is accepted only if it both supports structured outputs and has a published rate, so the analyzer can never recommend a model whose spend it could not account for. `claude-sonnet-4-6` and `claude-opus-4-7` stay priced but ungated — existing configs and older `cost.log` rows name them, and the installer migrates the 4.6 pin. `max_tokens` is a backstop at 16000, paired with a 300s curl timeout so the cap is reachable. **A day whose call did not produce a usable answer is held, not skipped**: a non-200, a transport failure, a `stop_reason` other than `end_turn`, or an empty response all leave `last-analyzed` before that day so it is retried, and the count lands in `runs/{date}.json` for the digest to render. Size rejections accumulate in `~/.claude/autoheal/rejected-days.jsonl`; after 7 rejections of the same day under the same analyzer version the day is skipped past. `--force-day YYYY-MM-DD` re-processes a single day without bumping `last-analyzed`. Caps are config-driven (`max_input_tokens`, `daily_cost_cap_usd`).
