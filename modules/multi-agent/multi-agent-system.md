...
Derived, git-ignored, and self-healing. Contains:
...
**Never edit this file, and never copy it between clones.** Every value is
derived from two things that cannot drift: the clone's own absolute path, and
`~/.claude/port-registry.json`. A `SessionStart` hook re-derives it at the
start of every session and rewrites it if it disagrees, so a hand-edit is
reverted rather than honoured. Keys the derivation does not own (project
secrets, device ids) are preserved untouched.

To inspect or fix by hand:

```bash
python3 ~/.claude/lib/clone_identity.py show              # this clone's derived identity
python3 ~/.claude/lib/clone_identity.py audit             # every clone; exit 1 on drift/collision
python3 ~/.claude/lib/clone_identity.py repair --all      # rewrite whatever disagrees
```

This replaced a design in which `.env.clone` was written once at clone-creation
time and then treated as the source of truth by every reader. When one clone's
file was copied to another, two clones claimed the same Vite port, Vite
silently auto-incremented onto a *third* clone's port, and `tracking.csv`
recorded work under the wrong agent id — none of it detectable from inside the
affected clone (issue #890).
...
- `.env.clone` is unique per clone and must NOT be copied between clones. If a
  bulk `cp .env*` ever does copy it, the next session's `clone-identity-sync`
  hook rewrites it from the clone's own path
...
for i in $(seq 0 $((AGENT_COUNT - 1))); do
  ...
  python3 ~/.claude/lib/clone_identity.py repair "$CLONE_DIR"
done
```

**Important**: Ensure `.env.clone` is in the project's `.gitignore`.

**Do not write `.env.clone` with `echo` or a heredoc.** Every hand-rolled
generator is another source of truth, and they disagree: three incompatible
`.env.clone` schemas were found in the wild before `clone_identity.py` became
the only writer (issue #890).
...
# 3. Derive identity and ports from the clone path + port registry
python3 ~/.claude/lib/clone_identity.py repair "$CLONE_DIR"

# 4. Copy env files. The .env.clone exclusion is load-bearing: a bare
#    `cp .env* ` here is what gave several clones the same identity.
...
`clones_per_workspace` is an optional per-repo field, default 4. Set it when a
repo's workspaces hold a different number of clones. It lives in the registry
rather than being counted from sibling directories because one stray or missing
directory would silently shift every port in the workspace.

**The registry is the sole authority.** Both `.env.clone` and the clone table in
each workspace `CLAUDE.md` are generated from it plus the clone path — neither
is hand-maintained, so neither can disagree with it. Regenerate a workspace
table with:

```bash
python3 ~/.claude/lib/clone_identity.py workspace-table ~/code/{repo}-workspaces/{repo}-w1
```

A repo missing from the registry has no derivable allocation. `repair` reports
it as `unregistered` and leaves the clone alone rather than inventing a
fallback base port, which is how two repos end up sharing 5173.
