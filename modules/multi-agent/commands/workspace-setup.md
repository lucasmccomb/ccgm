...
for w in $(seq 0 $((WORKSPACE_COUNT - 1))); do
  ...
  for c in $(seq 0 $((CLONE_COUNT - 1))); do
    ...
    if [ -d "$CLONE_DIR" ]; then
      echo "EXISTS: ${CLONE_DIR} (skipping clone; identity still re-derived below)"
    else
    ...
    # Write .env.clone from the clone's own path + the port registry. This runs
    # for pre-existing clones too: skipping them is what let a wrong .env.clone
    # survive every later re-run. `repair` is idempotent and preserves any
    # project-specific keys already in the file.
    python3 "$HOME/.claude/lib/clone_identity.py" repair "$CLONE_DIR"
  done
done
```

**Do not write `.env.clone` by hand or with a heredoc here.** Its contents are
derived from the clone path plus `~/.claude/port-registry.json`; a second
generator is a second source of truth, and that is exactly how the workspaces
drifted (issue #890).

If `repair` reports `unregistered`, the repo has no entry in
`~/.claude/port-registry.json`. Add one before continuing - pick a base port
block at least 16 above the highest existing entry, and set
`clones_per_workspace` when the workspace holds a number of clones other than
4:
...
"{repo-name}": {
  "frontend": 5397,
  "backend": 9011,
  "clones_per_workspace": {clone-count},
  "notes": "{what runs on these ports}"
}
```
...
**Generate the clone table; do not type it.** A hand-written table is a second
source of truth that drifts from the registry:

```bash
for w in $(seq 0 $((WORKSPACE_COUNT - 1))); do
  WORKSPACE_DIR="${WORKSPACES_DIR}/${REPO_NAME}-w${w}"
  python3 "$HOME/.claude/lib/clone_identity.py" workspace-table "$WORKSPACE_DIR"
done
```

Paste each workspace's generated table between the marker comments below, so a
later `clone_identity.py` run can refresh it in place.
...
<!-- BEGIN clone-identity-table -->
{output of: clone_identity.py workspace-table {workspace-dir}}
<!-- END clone-identity-table -->

Ports come from `~/.claude/port-registry.json` plus each clone's path. To check
or fix a clone: `python3 ~/.claude/lib/clone_identity.py audit` / `repair`.
...
Read ports from each clone's `.env.clone`. Never hardcode them, and never edit
that file: it is regenerated from the clone path plus the port registry
...
### Phase 11: Verify Identity and Ports

Before reporting success, prove every clone's identity and port allocation are
consistent. This exits non-zero on any drift or port collision:

```bash
python3 "$HOME/.claude/lib/clone_identity.py" audit --root "$HOME/code"
```

A `CLASH` line means two clones derive the same port. That is a registry
problem, not a per-clone one: the two repos' base-port blocks overlap, or a
repo has both a `-repos` flat setup and a `-workspaces` setup drawing from the
same block. Fix it by moving one block in `~/.claude/port-registry.json`, then
re-run `repair --all`. Do not paper over it by editing a `.env.clone`.
