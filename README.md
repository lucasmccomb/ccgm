...
|--------|----------|----------|-------------|--------------|
| **adversarial-review** | workflow | `/adrev` | Personal lead adversarial review by default; bounded opposite-provider review on explicit request. A designated writer applies supported fixes; report-only targets stay unchanged | subagent-patterns, cross-agent-review |
| **advisor-mode** | workflow | `/advisor` | Delegation-only session posture for expensive orchestrator models: the main agent specs, delegates, personally reviews spec/quality (cross-provider review is opt-in), and merges, while a hard PreToolUse gate blocks its own edits and non-orchestration Bash (subagent calls pass; ADVISOR_DIRECT=1 hatch). Per-session state, on by default at session start (CCGM_ADVISOR_AUTO=false opts out) | settings, subagent-patterns |
...
| **browser-automation** | patterns |  `/browser-automation`  | Browser tool selection (Chrome, Playwright, WebMCP), verification priority, UI testing workflow | - |
...
| **cross-agent-review** | workflow | `/cross-agent-review` | Opt-in Claude/Codex review with frozen evidence, bounded exchanges, evidence-bound acknowledgment reuse and reversible Codex skill setup | - |
...
| **git-worktrees** | workflow |  `/git-worktrees`, `/worktree-finish`, `/worktree-start`, `/worktree-sweep`  | Git worktrees as the default isolation for parallel sub-agent delegation on one machine, with a safe janitor (/worktree-sweep) that enforces teardown so worktrees never silently fill the disk | - |
...
| **model-vetting** | core |  `/model-vetting`  | Security vetting gate for new AI models: weights provenance, format safety, license/data terms, serving path, staged agentic access | - |
...
| **systematic-debugging** | patterns |  `/debugging-techniques`  | 4-phase root cause investigation: investigate, analyze, test hypotheses, implement fix | - |
...
| **xplan** | workflow | `/etp`, `/xplan`, `/xplan-resume`, `/xplan-status`, `/xplana` | Discovery, deep research, planning and execution with 1–3 upfront-selected adversarial passes (default one). Personal lead review and repository checks are the default; alternating Claude/Codex review is opt-in. Requires [/deepresearch](#companion-module-deepresearch) | multi-agent, adversarial-review, cross-agent-review |
